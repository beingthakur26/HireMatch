...
export function JobDetailModal({ job, resume, isOpen, isSaved, onSave, onClose }: JobDetailModalProps) {
                  ...
                  <div className="lg:col-span-2 space-y-10">
                    ...
                    {(coverLetter || improvements) && (
                      <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-500">
                        ...
                        {improvements && (
                          <div className="bg-emerald-50/50 rounded-xl p-8 border border-emerald-100">
                            <div className="flex items-center justify-between mb-6">
                              <h3 className="text-sm font-extrabold text-emerald-900 uppercase tracking-widest">ATS Optimization</h3>
                              <button onClick={() => setImprovements(null)} className="text-emerald-400 hover:text-emerald-600"><X className="w-4 h-4" /></button>
                            </div>
                            <div className="space-y-6">
                              <div>
                                <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-3">Priority Suggestions</p>
                                <ul className="space-y-3">
                                  {improvements.suggestions.map((s: any, i: number) => (
                                    <li key={i} className="flex gap-3 text-xs text-emerald-800">
                                      <TrendingUp className="w-4 h-4 shrink-0 text-emerald-500" />
                                      <div>
                                        <span className="font-bold block mb-1">{s.section}</span>
                                        <p className="font-medium opacity-80">{s.suggestion}</p>
                                      </div>
                                    </li>
                                  ))}
                                </ul>
                              ...
                              <div>
                                <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-3">Target Keywords</p>
                                <div className="flex flex-wrap gap-2">
                                  {improvements.atsKeywords.map((k: string) => (
                                    <span key={k} className="px-2 py-1 bg-white border border-emerald-200 rounded text-[10px] font-bold text-emerald-600 uppercase">
                                      {k}
                                    </span>
                                  ))}
                                </div>
